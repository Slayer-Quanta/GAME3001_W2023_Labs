...
glm::vec2 Agent::GetRighttimes2LOSEndPoint() const
{
	return m_righttimes2LOSEndPoint;
}

glm::vec2 Agent::GetLefttimes2LOSEndPoint() const
{
	return m_lefttimes2LOSEndpoint;
}
...
void Agent::SetRighttimes2LOSEndPoint(glm::vec2 point)
{
	m_righttimes2LOSEndPoint = point;
}

void Agent::SetLefttimes2LOSEndPoint(glm::vec2 point)
{
	m_lefttimes2LOSEndpoint = point;
}
...
void Agent::UpdateWhiskers(float angle)
{
	...
	x = sin((GetCurrentHeading() + m_whiskerAngle + 60) * Util::Deg2Rad);
	y = cos((GetCurrentHeading() + m_whiskerAngle + 60) * Util::Deg2Rad);
	SetRightLOSEndPoint(GetTransform()->position + glm::vec2(x, -y) * (GetLOSDistance() * 0.75f));

	x = sin((GetCurrentHeading() + m_whiskerAngle + 70) * Util::Deg2Rad);
	y = cos((GetCurrentHeading() + m_whiskerAngle + 70) * Util::Deg2Rad);
	SetRighttimes2LOSEndPoint(GetTransform()->position + glm::vec2(x, -y) * (GetLOSDistance() * 0.75f));

	 x = sin((GetCurrentHeading() - m_whiskerAngle + 80) * Util::Deg2Rad);
	 y = cos((GetCurrentHeading() - m_whiskerAngle + 80) * Util::Deg2Rad);
	SetLefttimes2LOSEndPoint(GetTransform()->position + glm::vec2(x, -y) * (GetLOSDistance() * 0.75f));
}
